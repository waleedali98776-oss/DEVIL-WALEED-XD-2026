...
if (!fs.existsSync("public")) fs.mkdirSync("public"); // <-- APNI DP IMAGE "public/logo.png" KE NAAM SE SAVE KAREIN
...
/* ============================================================
   WALEED — WHITE RGB MODERN THEME (DP LOGO)
   ============================================================ */
const THEME_CSS = `
:root{--rgb:linear-gradient(90deg,#ff0000,#ffaa00,#00ff00,#00ffff,#0000ff,#ff00ff,#ff0000)}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',system-ui,sans-serif;background:#ffffff;color:#141414;padding:20px}
...
background:linear-gradient(#fff,#fff) padding-box,var(--rgb) border-box;background-size:100% 100%,300% 100%;
animation:rgb 4s linear infinite;box-shadow:0 0 25px rgba(0,0,0,.15)}
...
.tag{color:#777;letter-spacing:3px;font-size:.75rem;text-transform:uppercase}
...
background:linear-gradient(#fff,#fff) padding-box,var(--rgb) border-box;background-size:100% 100%,300% 100%;
animation:rgb 5s linear infinite;box-shadow:0 4px 14px rgba(0,0,0,.06)}
.stat{padding:14px;text-align:center}
.stat b{display:block;font-size:1.4rem}
.stat span{color:#777;font-size:.72rem;letter-spacing:2px;text-transform:uppercase}
...
label{display:block;font-size:.72rem;color:#888;margin:10px 0 4px;letter-spacing:1px;text-transform:uppercase}
p{font-size:.85rem;color:#555;margin:6px 0}
ul{margin:8px 0 8px 20px;font-size:.85rem;color:#555}
li{margin:4px 0}
input,select{width:100%;padding:10px 12px;border:1px solid #ddd;border-radius:10px;background:#fafafa;font-size:.9rem;outline:none}
...
button:hover,.btn:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(0,0,0,.12)}
.console{margin-top:12px;background:#f7f7f7;border:1px solid #e5e5e5;border-radius:12px;height:180px;overflow-y:auto;
padding:12px;font-family:Consolas,monospace;font-size:.8rem;color:#333;white-space:pre-wrap}
.row{display:grid;grid-template-columns:1fr 1fr;gap:14px}
@media(max-width:700px){.row{grid-template-columns:1fr}}
.ok{color:#00a651;font-weight:700}
.err{color:#e00000;font-weight:700}
table{width:100%;border-collapse:collapse;margin:10px 0}
td,th{padding:8px;border-bottom:1px solid #eee;font-size:.85rem;text-align:left;vertical-align:top}
...
<h1>༒︎ 𝐖 𝐀 𝐋 𝐄 𝐄 𝐃 ༾︎</h1>
<div class="tag">ᴡʜᴀᴛꜱᴀᴘᴘ ꜱʏꜱᴛᴇᴍ ᴄᴏɴᴛʀᴏʟ ᴘᴀɴᴇʟ</div>
</header>
${content}
</div>
</body>
...
// ============ MAIN HOME — WALEED WHITE RGB MODERN ============
app.get("/", (req, res) => {
    res.send(themePage("༒︎  𝐀 𝐋 𝐄 𝐄 𝐃 ༾︎", `
<div class="stats">
